...
- `docredock --version`で現在のバージョンを確認できます。
- GUI更新確認は`DOCREDOCK_DISABLE_UPDATE_CHECK=1`で無効化できます。
- 編集していない文書を復元する場合は、元のバイト列をそのまま使用します。
- 非表示画像のOCRと選択ポリシー外の画像資産を安全側ポリシーから除外し、非表示XLSXセルを参照するグラフや過大なワークシート／グラフ範囲を保守的に扱うようにしました。

## 実験機能

PDF、roundtrip／audit出力、restore、render、diff、rebase、pack、unpack、migrateは実験機能です。配布版GUI／CLIでの実行には起動前の明示的な設定が必要です。
...
GitHub Releaseで各アーカイブのSHA-256と署名／notarization状況を確認してください。
