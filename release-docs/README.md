# DocRedock リリース文書

日本語 | [English](README.en.md)

版ごとのリリースノートと版に依存しない公開手順です。利用者向け入口は [README.ja.md](../README.ja.md) です。

## 最新版

### v0.2.9 Public Beta

- [v0.2.9 リリースノート](RELEASE_NOTES_v0.2.9.md)
- PDF工程表の横・縦の両矢印と、先端のセル位置を保持
- 未対応の斜め線を警告と原本照合で確認可能にし、不要な表の警告を削減
- 要確認ページと照合画像の件数を分けて表示し、OCR照合情報の読み順と表示量を改善

### v0.2.8 Public Beta

- [v0.2.8 リリースノート](RELEASE_NOTES_v0.2.8.md)
- PDF工程表の行・列と矢印方向の保持を改善
- 警告付き完了と原本照合用画像、OCRの信頼度・位置表示を追加
- 実験的な復元前チェックと元文書の上書き保護を追加

### v0.2.7 Public Beta

- [v0.2.7 リリースノート](RELEASE_NOTES_v0.2.7.md)
- 工程表の矢印や期間バーを、閲覧用Markdownの表で読み取りやすく改善
- Markdownの文字参照と、実験的な往復編集での本文保持を改善
- PDFの表認識とOCRの文字配置・利用環境の案内を改善

### v0.2.6 Public Beta

- [v0.2.6 リリースノート](RELEASE_NOTES_v0.2.6.md)
- 原文のリンク・画像・参照定義に似た文字列を、表示本文として保持
- 本物のリンク・画像・装飾・生成図を保ち、実験的なHTML表示のエスケープ処理を改善
- 入力と同じ実体を指すハードリンクへのCLI出力を拒否

### v0.2.5 Public Beta

- [v0.2.5 リリースノート](RELEASE_NOTES_v0.2.5.md)
- 入力原本の上書きを防ぎ、スタイル・グループ由来の非表示内容を除外
- Wordの本文・数式・入れ子表、PowerPointのマスター文字と表、Excelの表示形式を改善
- PDFの本文・混在画像・段組みの読み順を保持し、複数表の誤警告を削減
- 原文の記号をMarkdown構文から保護し、実験的な往復編集での本文保持を改善

### v0.2.4 Public Beta

- [v0.2.4 リリースノート](RELEASE_NOTES_v0.2.4.md)
- PDFの表外本文を保持し、二段組の読み順と複数罫線パスの表再構成を改善
- XLSXのシート指定を厳密化し、doctorの終了コードとstrict判定を明確化
- CLIとGUIの図・fallback集計を一致させ、誤警告を削減

### v0.2.3 Public Beta

- [v0.2.3 リリースノート](RELEASE_NOTES_v0.2.3.md)
- 規則的なPDF罫線表をMarkdown表へ変換し、セル文字列の重複を抑制
- 図の小さな隙間やずれへの許容を維持し、未解決の関係は注記として保持
- 大量の図形・診断の出力上限、実際の利用環境を調べるdoctor、PDF OCR連携を追加

### v0.2.2 Public Beta

- [v0.2.2 リリースノート](RELEASE_NOTES_v0.2.2.md)
- PDFの表罫線、矢印方向、分岐ラベルの認識を改善
- XLSXの数式警告と選択シートの出力範囲を改善
- CLI診断を読みやすく集約し、形式別の品質確認を強化

### v0.2.1 Public Beta

- [v0.2.1 リリースノート](RELEASE_NOTES_v0.2.1.md)
- アプリ内の現在バージョン表示、起動時／手動のPublic Betaを含む公開版更新確認、信頼済みリリースページへの導線
- PDF表罫線の抑制と矢印方向保持、診断とXLSX投影の精度改善
- `docredock` launcherとLinux install/uninstallを含む配布導線

### v0.2.0 Public Beta

- [v0.2.0 リリースノート](RELEASE_NOTES_v0.2.0.md)
- [リリース証跡の訂正](RELEASE_EVIDENCE_CORRECTION_v0.2.0.md)
- 視覚接続の推定は安全優先（safe）を既定とし、未解決の関係は fallback と診断に残す
- [v0.1.7 正誤表](ERRATA_v0.1.7.md)

### v0.1.7 Public Beta

- [v0.1.7 リリースノート](RELEASE_NOTES_v0.1.7.md)
- [v0.1.7 正誤表](ERRATA_v0.1.7.md)
- [v0.1.6 正誤表](ERRATA_v0.1.6.md)
- 不確実な図の関係は確定せず、fallback と診断で確認可能にする

## 利用者向け

- [利用ガイド](../docs/ja/user-guide.md)
- [対応機能](../docs/ja/supported-features.md)
- [実験機能](../docs/ja/experimental-features.md)
- [セキュリティとプライバシー](../docs/ja/security-and-privacy.md)

## 保守者向け

- [公開範囲](PUBLICATION_SCOPE.md)
- [リリースチェックリスト](RELEASE_CHECKLIST.md)
- [English publication scope](PUBLICATION_SCOPE.en.md)
- [English release checklist](RELEASE_CHECKLIST.en.md)
