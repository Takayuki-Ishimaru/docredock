#!/usr/bin/env python3
"""Assemble the PDF semantic portion of v0.2.9 package-smoke evidence."""
...
def run_source_suite(cli: Path, output: Path) -> int:
    ...
    document = {"schema_version": 1, "version": "0.2.9", "status": "pass" if not errors else "fail", "errors": errors,
...
def main() -> int:
    ...
    for path in args.smoke_evidence:
        ...
        if version != "0.2.9" or not isinstance(commit, str) or len(commit) < 7 or commit == "local":
    ...
    output = {"schema_version": 1, "version": "0.2.9", "cases": records, "source_suite": source_suite,
