...
The codebase also contains experimental CLI PDF, round-trip, restore, and render paths. Their presence does not make them supported in v0.1.6; CLI access requires `DOCREDOCK_ENABLE_EXPERIMENTAL=1`. The desktop GUI accepts PDF input by default, while PDF OCR still needs configured providers. See the [release support table](docs/en/supported-features.md).
