...
public sealed class XlsxAdapterTests
{
    ...
    [Fact]
    public void CoordinateTableCompletesXlsxToMarkdownEditToXlsxRoundTrip()
    {
        var original = CreatePackage();
        var adapter = new XlsxAdapter();
        ...
        Assert.Contains("rtmd:sheet-table range=A1:C1", markdown);
        Assert.Contains("| Row | A | B | C |", markdown);
    ...
    public void Sequence_grid_is_projected_as_protected_mermaid_before_round_trip_table()
    {
        ...
        var diagram = Assert.Single(extraction.Graph.Nodes, node => node.Kind == Rtmd.Core.Documents.NodeKind.Diagram);
        var source = Assert.IsType<Rtmd.Core.Documents.TextNodeContent>(diagram.Content).Text;
        ...
        Assert.Equal(Rtmd.Core.Documents.NodeEditability.Protected, diagram.Editability);

        var markdown = new RtmdMarkdownSerializer().Serialize(extraction.Graph).Markdown;
        Assert.Contains("```mermaid\nsequenceDiagram", markdown);
        Assert.True(markdown.IndexOf("```mermaid", StringComparison.Ordinal) < markdown.IndexOf("rtmd:sheet-table", StringComparison.Ordinal));
    ...
    public void Sequence_projection_preserves_source_numbers_fragments_activations_and_branch_annotations()
    {
        ...
        var source = Assert.IsType<Rtmd.Core.Documents.TextNodeContent>(
            Assert.Single(extraction.Graph.Nodes, node => node.Kind == Rtmd.Core.Documents.NodeKind.Diagram).Content).Text;
