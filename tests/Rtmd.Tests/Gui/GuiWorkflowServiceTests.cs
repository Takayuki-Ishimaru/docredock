...
public sealed class GuiWorkflowServiceTests
{
    ...
    public async Task Readable_export_creates_only_markdown_without_a_restore_package()
    {
        ...
        Assert.DoesNotContain("rtmd_schema", await File.ReadAllTextAsync(exported.MarkdownPath), StringComparison.Ordinal);
    }
    ...
    [Fact]
    public async Task Export_and_restore_round_trip_an_edited_docx_through_portable_files()
    {
        using var fixture = new Fixture();
        var source = Path.Combine(fixture.Root, "proposal.docx");
        ...
        Assert.True(File.Exists(exported.PackagePath));
        Assert.False(Directory.Exists(Path.ChangeExtension(exported.MarkdownPath, ".rtmd")));
        Assert.Equal(
            new[] { exported.MarkdownPath, exported.PackagePath }.Order(StringComparer.Ordinal),
    ...
    public async Task Export_can_choose_a_numbered_name_for_repeated_gui_runs()
    {
        ...
        Assert.EndsWith(" (2).rtmdpkg", second.PackagePath, StringComparison.Ordinal);
    }
    ...
    {
        public string Root { get; } = Path.Combine(Path.GetTempPath(), "rtmd-gui-tests", Guid.NewGuid().ToString("N"));
