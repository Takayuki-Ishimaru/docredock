...
public sealed class RtmdMarkdownEditPolicyTests
{
    [Theory]
    ...
    public void Advertises_only_edits_supported_by_the_built_in_restore_path(
        ...
        string operation)
    {
        var policy = RtmdMarkdownEditPolicy.For(format, Node(kind));
    ...
    {
        var policy = RtmdMarkdownEditPolicy.For(DocumentFormatKind.Docx, Node(NodeKind.Paragraph, editability));
    ...
    {
        var policy = RtmdMarkdownEditPolicy.For(
    ...
    public void Serializer_emits_policy_as_hidden_marker_attributes()
    {
        ...
        Assert.Contains("rtmd_rules: 1.0", markdown);
