...
public sealed class MarkdownRendererTests
{
    ...
    public async Task Mermaid_block_is_rendered_as_a_native_docx_image()
    {
        ...
        using var archive = ZipFile.OpenRead(output);
        Assert.NotNull(archive.GetEntry("word/media/rtmd-mermaid-1.png"));
        Assert.Contains("rIdRtmdMermaid1", await ReadEntryAsync(archive, "word/_rels/document.xml.rels"));
    ...
    public async Task Mermaid_block_is_rendered_as_a_native_pptx_picture()
    {
        ...
        using var archive = ZipFile.OpenRead(output);
        Assert.NotNull(archive.GetEntry("ppt/media/rtmd-mermaid-1.png"));
        Assert.Contains("rIdRtmdMermaid1", await ReadEntryAsync(archive, "ppt/slides/_rels/slide1.xml.rels"));
    ...
    public async Task Mermaid_block_is_rendered_as_a_native_xlsx_drawing()
    {
        ...
        using var archive = ZipFile.OpenRead(output);
        Assert.NotNull(archive.GetEntry("xl/media/rtmd-mermaid-1.png"));
        Assert.NotNull(archive.GetEntry("xl/drawings/drawing1.xml"));
        Assert.Contains("rIdRtmdDrawing1", await ReadEntryAsync(archive, "xl/worksheets/_rels/sheet1.xml.rels"));
        Assert.Contains("rIdRtmdMermaid1", await ReadEntryAsync(archive, "xl/drawings/_rels/drawing1.xml.rels"));
    ...
    [Fact]
    public async Task Rtmd_projection_control_metadata_is_not_rendered_into_docx()
    {
        using var fixture = new Fixture();
        ...
        Assert.DoesNotContain("rtmd:", xml);
        Assert.DoesNotContain("rtmd_schema", xml);
        Assert.DoesNotContain("document_id", xml);
    }

    [Fact]
    public async Task Incomplete_rtmd_projection_is_rejected_by_render()
    {
        using var fixture = new Fixture();
        var output = Path.Combine(fixture.Root, "rendered.docx");
        const string projection = """
    ...
    [Fact]
    public void Generic_markdown_code_examples_with_rtmd_markers_are_not_misclassified_or_removed()
    {
        const string generic = """
            # Parser example

            ```md
            <!--rtmd:delete id=example-->
            ```
            """;

        Assert.False(RtmdProjectionCleaner.IsRtmdProjection(generic));
        Assert.Equal(generic, RtmdProjectionCleaner.Clean(generic));
    }
    ...
    public async Task Mermaid_template_render_merges_relationships_and_parts_without_collisions(RenderFormat format, string extension)
    {
        ...
        switch (format)
        {
            case RenderFormat.Docx:
            {
                ...
                Assert.Equal("rIdRtmdMermaid1_2", imageId);
                ...
                Assert.Equal("media/rtmd-mermaid-1-2.png", (string?)image.Attribute("Target"));
                Assert.NotNull(rendered.GetEntry("word/media/rtmd-mermaid-1.png"));
                Assert.NotNull(rendered.GetEntry("word/media/rtmd-mermaid-1-2.png"));
            ...
            case RenderFormat.Pptx:
            {
                ...
                var image = rels.Descendants(packageRelationships + "Relationship").Single(element => (string?)element.Attribute("Id") == imageId);
                Assert.Equal("../media/rtmd-mermaid-1-2.png", (string?)image.Attribute("Target"));
                Assert.NotNull(rendered.GetEntry("ppt/media/rtmd-mermaid-1.png"));
                Assert.NotNull(rendered.GetEntry("ppt/media/rtmd-mermaid-1-2.png"));
            ...
            case RenderFormat.Xlsx:
            {
                ...
                Assert.Equal("rIdRtmdDrawing1_2", drawingId);
                ...
                Assert.Equal("../drawings/rtmd-drawing1.xml", (string?)drawingRelationship.Attribute("Target"));
                Assert.NotNull(rendered.GetEntry("xl/drawings/drawing1.xml"));
                Assert.NotNull(rendered.GetEntry("xl/drawings/rtmd-drawing1.xml"));

                var drawing = XDocument.Parse(await ReadEntryAsync(rendered, "xl/drawings/rtmd-drawing1.xml"));
                var imageId = (string?)drawing.Descendants().Attributes(relationships + "embed").Single();
                var drawingRels = XDocument.Parse(await ReadEntryAsync(rendered, "xl/drawings/_rels/rtmd-drawing1.xml.rels"));
                var image = drawingRels.Descendants(packageRelationships + "Relationship").Single(element => (string?)element.Attribute("Id") == imageId);
                Assert.Equal("../media/rtmd-mermaid-1-2.png", (string?)image.Attribute("Target"));
                Assert.NotNull(rendered.GetEntry("xl/media/rtmd-mermaid-1.png"));
                Assert.NotNull(rendered.GetEntry("xl/media/rtmd-mermaid-1-2.png"));
                ...
                Assert.Contains(types.Descendants(contentTypes + "Override"), element =>
                    (string?)element.Attribute("PartName") == "/xl/drawings/rtmd-drawing1.xml");
    ...
    {
        public string Root { get; } = Path.Combine(Path.GetTempPath(), "rtmd-render-tests", Guid.NewGuid().ToString("N"));
