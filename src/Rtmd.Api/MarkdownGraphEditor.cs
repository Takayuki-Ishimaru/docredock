...
/// <summary>Maps a RTMD projection back to the canonical graph without using text heuristics for identity.</summary>
public sealed class MarkdownGraphEditor
{
    public GraphEditResult Apply(DocumentGraph baseline, string markdown)
    {
        ...
        foreach (var addition in parsed.Blocks.Where(block => block.IsNew))
        {
            ...
            if (!SupportsMarkdownAddition(baseline.Format, addition.Kind))
                diagnostics.Add(new Diagnostic("UnsupportedMarkdownAddition", $"RTMD Markdown cannot safely add kind '{addition.Kind}' to {baseline.Format.ToString().ToLowerInvariant()} with the built-in restore path.", DiagnosticSeverity.Error));
        }
        ...
        diagnostics.AddRange(RtmdMarkdownParser.FindMissingNodes(parsed, baseline.Nodes
        ...
        foreach (var partition in baseline.Partitions.OrderBy(partition => partition.Order))
        {
            ...
            foreach (var node in partition.Nodes.OrderBy(node => node.Order))
            {
                ...
                if (node.Editability is NodeEditability.Protected or NodeEditability.Passthrough)
                {
                    if (!ProtectedContentMatches(node, block))
    ...
    private static DocumentNode ApplyBlock(DocumentNode node, TypedMarkdownBlock block)
    {
        if (node.Content is RichTextNodeContent rich)
        {
            ...
            if (StringComparer.Ordinal.Equals(RtmdInlineMarkdown.Serialize(rich.Runs), inline)) return node;
            return node with { Content = RtmdInlineMarkdown.Parse(inline, rich.Runs) };
        }
    ...
    private static bool ProtectedContentMatches(DocumentNode node, TypedMarkdownBlock block)
    {
        if (node.Content is RichTextNodeContent rich)
            return StringComparer.Ordinal.Equals(RtmdInlineMarkdown.Serialize(rich.Runs), DecodeInlineBlockMarkdown(block));
        ...
        if (node.Content is TableNodeContent table)
        {
            ...
            return table.Rows.Count == projected.Rows.Count && table.Rows.Zip(projected.Rows).All(pair =>
                pair.First.Count == pair.Second.Count && pair.First.Zip(pair.Second).All(cell =>
                    StringComparer.Ordinal.Equals(cell.First, cell.Second)));
        }
        ...
        ICollection<Diagnostic> diagnostics)
    {
        ...
        if (!keys.Any(block.Attributes.ContainsKey)) return; // RTMD Markdown 1.0 legacy projection.
        var policy = RtmdMarkdownEditPolicy.For(format, node);
        ...
        ICollection<Diagnostic> diagnostics)
    {
                     ...
                     .GroupBy(block => block.PartitionId!, StringComparer.Ordinal))
        {
            ...
            foreach (var block in group)
            {
                ...
                var sectionValues = ParseSheetGrid(block.Text, sectionAddresses, diagnostics);
