...
using Rtmd.Formats.OpenXml.Xlsx;
...
public sealed class DocumentService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var ocrResults = await CollectOcrAsync(format, graph, assets, options.EnableOcr,
            options.OcrLanguages ?? ["jpn", "eng"], diagnostics, cancellationToken).ConfigureAwait(false);
        return (AttachAssetsAndOcr(graph, assets, ocrResults), diagnostics, assets);
    }
    ...
    {
        var workspace = await RoundTripWorkspace.OpenAsync(workspacePath, cancellationToken).ConfigureAwait(false);
        var projectionPath = markdownPath ?? ProjectionPath(workspace);
        ...
        return new(baseline, edit, edit.Diagnostics);
    }
    ...
    {
        ArgumentNullException.ThrowIfNull(options);
        var workspace = await RoundTripWorkspace.OpenAsync(options.WorkspacePath, cancellationToken).ConfigureAwait(false);
        ...
        var diff = await DiffAsync(options.WorkspacePath, options.MarkdownPath, cancellationToken).ConfigureAwait(false);
        if (!diff.Edit.IsValid) return new(output, FidelityLevel.FX, false, diff.Diagnostics);
        var markdown = options.MarkdownPath ?? ProjectionPath(workspace);
        ...
        switch (workspace.Manifest.Source.Format.ToLowerInvariant())
        {
            case "docx":
                {
                    ...
                    return new(output, result.Fidelity.Level, result.Succeeded, result.Diagnostics);
                }
            case "xlsx":
                {
                    if (diff.Edit.Diff.PatchSet.Operations.Any(operation => operation.MutatesOriginal &&
                        ...
                        return new(output, FidelityLevel.FX, false,
                            [new("UnsupportedXlsxPatch", "XLSX F1 supports cell value/formula updates and additions, not deletion or structural edits.", DiagnosticSeverity.Error)]);
                    await using var input = File.OpenRead(source);
                    var plan = xlsx.CreatePatchPlan(diff.Baseline, diff.Edit.EditedGraph);
                    if (plan.Edits.Count == 0)
                        return new(output, FidelityLevel.FX, false,
                            [new("UnplannedXlsxChange", "The XLSX DirtySet could not be represented by the cell patch planner.", DiagnosticSeverity.Error)]);
                    ...
                    return new(output, FidelityLevel.F1, true, diagnostics);
                }
            case "pptx":
                {
                    if (diff.Edit.Diff.PatchSet.Operations.Any(operation => operation.MutatesOriginal &&
                        ...
                        return new(output, FidelityLevel.FX, false,
                            [new("UnsupportedPptxPatch", "PPTX F1 supports existing shape text replacement only.", DiagnosticSeverity.Error)]);
                    await using var input = File.OpenRead(source);
                    var plan = pptx.CreatePatchPlan(diff.Baseline, diff.Edit.EditedGraph);
                    if (plan.Edits.Count == 0)
                        return new(output, FidelityLevel.FX, false,
                            [new("UnplannedPptxChange", "The PPTX DirtySet could not be represented by the shape patch planner.", DiagnosticSeverity.Error)]);
            ...
            case "pdf" when options.AllowRenderFallback:
                ...
                return new(output, FidelityLevel.F3, true, [new("PdfRendered", "Edited PDF was rendered as a new PDF.", DiagnosticSeverity.Warning)]);
            case "pdf":
                return new(output, FidelityLevel.FX, false, [new("RenderFallbackRequired", "Edited PDF requires explicit AllowRenderFallback.", DiagnosticSeverity.Error)]);
            default: return new(output, FidelityLevel.FX, false, [new("UnsupportedFormat", "Workspace format is unsupported.", DiagnosticSeverity.Error)]);
        }
    }
    ...
    private static ProviderSet ProvidersFor(DocumentFormatKind format, ProviderDescriptor? ocrDescriptor) => new()
    {
        ...
        {
            DocumentFormatKind.Docx => "rtmd.docx.openxml",
            DocumentFormatKind.Xlsx => "rtmd.xlsx.openxml",
            DocumentFormatKind.Pptx => "rtmd.pptx.openxml",
            DocumentFormatKind.Pdf => "rtmd.pdf.builtin",
            _ => "rtmd.adapter.none",
        }, "0.2.0", 1),
        Markdown = new ProviderInfo("rtmd.markdown.default", "0.2.0", 1),
        Ocr = ocrDescriptor is null ? new ProviderInfo("rtmd.ocr.none", "0.2.0", 1) : new ProviderInfo(ocrDescriptor.ProviderId, ocrDescriptor.ProviderVersion.ToString(), ocrDescriptor.InterfaceVersion) { Sha256 = ocrDescriptor.BinarySha256 }
    };
        ...
        IReadOnlyList<OcrAssetRecord> ocrResults)
    {
        ...
        foreach (var item in completed)
        {
            var parent = graph.Nodes.FirstOrDefault(node => node.Kind == NodeKind.Image &&
                ...
                assets.FirstOrDefault(asset => asset.Id == item.AssetId) is { } asset &&
                (reference.Reference.EndsWith(Path.GetFileName(asset.SourcePartUri ?? string.Empty), StringComparison.OrdinalIgnoreCase) ||
                 StringComparer.Ordinal.Equals(reference.Reference, item.AssetId)));
    ...
    private static DocumentGraph AttachAssetReferences(DocumentGraph graph, IReadOnlyList<WorkspaceAsset> assets)
    {
        ...
        var partitions = graph.Partitions.Select(partition => partition with
        {
            Nodes = partition.Nodes.Select(node =>
            {
                ...
                return node with { Content = reference with { Reference = asset.Id } };
            }).ToArray()
        }).ToList();
    ...
    private static WorkspaceAsset? FindAsset(string reference, IReadOnlyList<WorkspaceAsset> assets)
    {
        ...
        {
            var source = (asset.SourcePartUri ?? string.Empty).Replace('\\', '/').TrimStart('/');
        ...
        var nameMatches = assets.Where(asset => StringComparer.OrdinalIgnoreCase.Equals(
            Path.GetFileName(asset.SourcePartUri ?? asset.FileName), fileName)).ToArray();
    ...
    private static async Task<IReadOnlyList<WorkspaceAsset>> ExtractOfficeAssetsAsync(string sourcePath, CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in archive.Entries.Where(entry => entry.FullName.Contains("/media/", StringComparison.OrdinalIgnoreCase)).OrderBy(entry => entry.FullName, StringComparer.Ordinal))
        {
            ...
            result.Add(new WorkspaceAsset($"img-{++index:D4}", $"img-{index:D4}{extension}", MediaType(extension), hash, bytes, "/" + entry.FullName));
        }
        return result;
    }
    ...
    private static string ProjectionPath(RoundTripWorkspace workspace) => Path.Combine(Directory.GetParent(workspace.RootPath)?.FullName ?? workspace.RootPath, workspace.Manifest.Projection.FileName);
    ...
    private static string MediaType(string extension) => extension switch { ".png" => "image/png", ".jpg" or ".jpeg" => "image/jpeg", ".gif" => "image/gif", ".webp" => "image/webp", ".bmp" => "image/bmp", ".tif" or ".tiff" => "image/tiff", _ => "application/octet-stream" };
