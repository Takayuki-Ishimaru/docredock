...
public sealed record MarkdownSerializationOptions
{
    ...
    public string RoundTripStore { get; init; } = "document.rtmd";
...
/// <summary>A typed, provider-neutral view of a RTMD Markdown block.</summary>
...
/// <summary>
/// Deterministic RTMD projection serializer. The graph argument is intentionally
...
/// </summary>
public sealed class RtmdMarkdownSerializer
{
    ...
    public MarkdownProjection Serialize(object graph, MarkdownSerializationOptions? options = null)
    {
        ...
        {
            output.AppendLine("---");
            output.Append("rtmd_schema: ").AppendLine(options.SchemaVersion);
            output.Append("rtmd_rules: ").AppendLine(options.RulesVersion);
            ...
            output.AppendLine("preserve_rtmd_comments: true");
            output.AppendLine("---");
        }
        ...
        foreach (var partition in partitions.OrderBy(p => p.Order).ThenBy(p => p.Id, StringComparer.Ordinal))
        {
            var nodes = partition.Nodes
                ...
                .ToList();
            output.Append("<!--rtmd:partition-begin id=").Append(EscapeAttribute(partition.Id))
            ...
            foreach (var node in nodes)
            {
                ...
                output.Append("<!--rtmd:block id=").Append(EscapeAttribute(blockId)).Append(" kind=")
            ...
            output.Append("<!--rtmd:partition-end id=").Append(EscapeAttribute(partition.Id))
                .Append(" baseline_nodes=").Append(nodes.Count.ToString(CultureInfo.InvariantCulture)).AppendLine("-->");
        }

        output.Append("<!--rtmd:document-end id=").Append(EscapeAttribute(documentId))
    ...
    public MarkdownProjection Serialize(DocumentGraph graph, MarkdownSerializationOptions? options = null)
    {
        ...
        var assetReferences = graph.Assets?.ToDictionary(
            item => item.Key,
            item => options.RoundTripStore + "/assets/" + (item.Value.FileName ?? item.Key),
        ...
        void AppendNodeBlock(DocumentNode node)
        {
            ...
            var editPolicy = RtmdMarkdownEditPolicy.For(graph.Format, node);
            output.Append("<!--rtmd:block id=").Append(EscapeAttribute(node.Id))
            ...
            output.AppendLine();
            AddContribution(contributions, options.ProjectionId, node.Id, new TextRange(start, length), node.Id, RoleFor(node), EvidenceFor(node));
        }
        ...
        foreach (var partition in partitions)
        {
            ...
            output.Append("<!--rtmd:partition-begin id=").Append(EscapeAttribute(partition.Id)).Append(" baseline_nodes=")
            ...
            {
                foreach (var section in SplitSheetSections(sheetCells))
                {
                    var range = SheetRange(section);
                    output.Append("<!--rtmd:sheet-table range=").Append(range)
            ...
            foreach (var node in standaloneNodes.Where(node => !IsMermaidDiagram(node))) AppendNodeBlock(node);
            output.Append("<!--rtmd:partition-end id=").Append(EscapeAttribute(partition.Id)).Append(" baseline_nodes=")
                .Append(nodes.Length.ToString(CultureInfo.InvariantCulture)).AppendLine("-->");
        }
        output.Append("<!--rtmd:document-end id=").Append(EscapeAttribute(graph.DocumentId)).Append(" partitions=")
            .Append(partitions.Length.ToString(CultureInfo.InvariantCulture)).AppendLine("-->");
        return new MarkdownProjection(options.ProjectionId, output.ToString(), contributions, Array.Empty<MarkdownDiagnostic>());
    }
    ...
    private static void AppendCoreNode(StringBuilder output, DocumentNode node, Func<string, string> resolveImageReference)
    {
        var text = node.Content is RichTextNodeContent rich
            ? RtmdInlineMarkdown.Serialize(rich.Runs)
    ...
    private static void AppendSheetTable(StringBuilder output, IReadOnlyList<SpreadsheetCell> cells)
    {
        ...
        output.Append("| Row |");
        foreach (var column in columns) output.Append(' ').Append(ColumnName(column)).Append(" |");
        output.AppendLine();
        output.Append("| --- |");
        ...
        {
            output.Append("| ").Append(row.ToString(CultureInfo.InvariantCulture)).Append(" |");
    ...
    private static void AppendTable(StringBuilder output, IReadOnlyList<IReadOnlyList<string>> rows)
    {
        ...
        {
            output.Append('|');
            for (var index = 0; index < width; index++) output.Append(' ').Append(EscapeTableCell(index < row.Count ? row[index] : string.Empty)).Append(" |");
            output.AppendLine();
        }
    }
    private static string ImageReference(string reference, string store) => reference.StartsWith("asset:", StringComparison.Ordinal) ? store + "/assets/" + reference[6..] : reference;
...
public sealed class RtmdMarkdownParser
{
    private static readonly Regex FrontMatter = new("^---\\r?\\n(?<body>.*?)\\r?\\n---\\r?\\n", RegexOptions.Singleline | RegexOptions.Compiled);
    private static readonly Regex Marker = new("<!--rtmd:(?<kind>block|delete|new|sheet-table|partition-begin|partition-end|document-end)(?:\\s+(?<attrs>.*?))?-->", RegexOptions.Compiled);
    ...
    public TypedMarkdownDocument Parse(string markdown, MarkdownParseOptions? options = null)
    {
        ...
        var values = ParseAttributes(front.Success ? front.Groups["body"].Value.Replace(": ", "=", StringComparison.Ordinal) : "");
        if (options.RequireFrontMatter && !front.Success)
            diagnostics.Add(new("RTMD001", "Front matter is missing.", MarkdownDiagnosticSeverity.Error));

        var documentId = Get(values, "document_id");
        var schema = Get(values, "rtmd_schema");
        var rulesVersion = Get(values, "rtmd_rules");
        var sourceFormat = Get(values, "source_format");
        var store = Get(values, "roundtrip_store");
        if (string.IsNullOrWhiteSpace(documentId))
            diagnostics.Add(new("RTMD011", "Front matter document_id is missing.", MarkdownDiagnosticSeverity.Error));
        if (!StringComparer.Ordinal.Equals(schema, "1.0"))
            diagnostics.Add(new("RTMD012", $"Unsupported RTMD Markdown schema '{schema ?? "(missing)"}'.", MarkdownDiagnosticSeverity.Error));
        if (rulesVersion is not null && !StringComparer.Ordinal.Equals(rulesVersion, "1.0"))
            diagnostics.Add(new("RTMD022", $"Unsupported RTMD AI editing rules version '{rulesVersion}'.", MarkdownDiagnosticSeverity.Error));
        if (string.IsNullOrWhiteSpace(sourceFormat))
            diagnostics.Add(new("RTMD013", "Front matter source_format is missing.", MarkdownDiagnosticSeverity.Error));
            ...
            switch (marker.Kind)
            {
                case "partition-begin":
                    ...
                    if (openPartition is not null)
                        diagnostics.Add(new("RTMD017", "Partition begin marker is nested inside another partition.", MarkdownDiagnosticSeverity.Error, beginId));
                    if (string.IsNullOrWhiteSpace(beginId))
                        diagnostics.Add(new("RTMD018", "Partition begin marker has no id.", MarkdownDiagnosticSeverity.Error));
                    else if (partitionCounts.ContainsKey(beginId))
                        diagnostics.Add(new("RTMD014", $"Partition '{beginId}' is declared more than once.", MarkdownDiagnosticSeverity.Error, beginId));
                    else { openPartition = beginId; partitionCounts[beginId] = 0; }
                    break;
                case "partition-end":
                    var endId = Get(attrs, "id");
                    if (openPartition is null)
                        diagnostics.Add(new("RTMD010", $"Partition '{endId ?? "(missing)"}' has no matching begin marker.", MarkdownDiagnosticSeverity.Error, endId));
                    else if (!StringComparer.Ordinal.Equals(openPartition, endId))
                    {
                        diagnostics.Add(new("RTMD019", $"Partition end '{endId ?? "(missing)"}' does not match open partition '{openPartition}'.", MarkdownDiagnosticSeverity.Error, endId));
                        openPartition = null;
                    }
                    else
                    {
                ...
                case "document-end":
                    if (openPartition is not null)
                        diagnostics.Add(new("RTMD008", $"Partition '{openPartition}' has no matching end marker.", MarkdownDiagnosticSeverity.Error, openPartition));
                    documentEnd = marker;
                    break;
                case "block":
                case "new":
                ...
                case "sheet-table":
                    if (openPartition is null)
                    {
                        diagnostics.Add(new("RTMD020", $"RTMD {marker.Kind} marker must appear inside a partition.", MarkdownDiagnosticSeverity.Error));
                        break;
                    }
                    if (marker.Kind == "sheet-table")
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(range))
                            diagnostics.Add(new("RTMD023", "Sheet table marker has no range.", MarkdownDiagnosticSeverity.Error));
                        if (!int.TryParse(Get(attrs, "baseline_nodes"), NumberStyles.Integer, CultureInfo.InvariantCulture, out var covered) || covered < 1)
                        {
                            diagnostics.Add(new("RTMD024", "Sheet table marker has an invalid baseline_nodes count.", MarkdownDiagnosticSeverity.Error));
                            covered = 0;
                        }
                    ...
                    {
                        var id = Get(attrs, "id");
                        if (string.IsNullOrWhiteSpace(id)) diagnostics.Add(new("RTMD002", "Delete marker has no id.", MarkdownDiagnosticSeverity.Error));
                        else if (!declaredIds.Add(id)) diagnostics.Add(new("RTMD003", $"Duplicate block id '{id}'.", MarkdownDiagnosticSeverity.Error, id));
                    ...
                    {
                        if (string.IsNullOrWhiteSpace(blockId)) diagnostics.Add(new("RTMD002", "Block marker has no id.", MarkdownDiagnosticSeverity.Error));
                        else if (!declaredIds.Add(blockId)) diagnostics.Add(new("RTMD003", $"Duplicate block id '{blockId}'.", MarkdownDiagnosticSeverity.Error, blockId));
                        partitionCounts[openPartition]++;
                    }
        ...
        if (options.RequireDocumentEnd && !complete)
            diagnostics.Add(new("RTMD004", "Document-end marker is missing; input may be truncated.", MarkdownDiagnosticSeverity.Error));
        if (complete)
        {
            if (markers.Count(marker => marker.Kind == "document-end") != 1)
                diagnostics.Add(new("RTMD015", "Document-end marker must appear exactly once.", MarkdownDiagnosticSeverity.Error));
            var endMarker = documentEnd!;
            if (!string.IsNullOrWhiteSpace(markdown[(endMarker.Index + endMarker.Length)..]))
                diagnostics.Add(new("RTMD016", "Content appears after the document-end marker.", MarkdownDiagnosticSeverity.Error));
            ...
            if (documentId is not null && markerDocumentId is not null && !StringComparer.Ordinal.Equals(documentId, markerDocumentId))
                diagnostics.Add(new("RTMD005", "Document ID does not match document-end marker.", MarkdownDiagnosticSeverity.Error));
            if (int.TryParse(Get(attrs, "partitions"), NumberStyles.Integer, CultureInfo.InvariantCulture, out var expectedPartitions) &&
                ...
                diagnostics.Add(new("RTMD006", "Partition count does not match document-end marker.", MarkdownDiagnosticSeverity.Error));
        }
    ...
    public static IReadOnlyList<MarkdownDiagnostic> FindMissingNodes(TypedMarkdownDocument document, IEnumerable<string> baselineNodeIds)
    {
        ...
        return baselineNodeIds.Where(id => !present.Contains(id) && !deleted.Contains(id))
            .Select(id => new MarkdownDiagnostic("RTMD007", $"Baseline node '{id}' is missing; it will be preserved, not deleted.", MarkdownDiagnosticSeverity.Warning, id))
            .ToArray();
    }
    ...
    private static void ValidateBaselineCount(IReadOnlyDictionary<string, string> attrs, int actual, string partitionId, List<MarkdownDiagnostic> diagnostics)
    {
        if (int.TryParse(Get(attrs, "baseline_nodes"), NumberStyles.Integer, CultureInfo.InvariantCulture, out var expected) && expected != actual)
            diagnostics.Add(new("RTMD021", $"Partition '{partitionId}' baseline_nodes is {expected}, but contains {actual} existing block markers.", MarkdownDiagnosticSeverity.Error, partitionId));
    }
