...
public static class RoundTripPackage
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var workspacePath = Path.Combine(Path.GetDirectoryName(markdownPath)!, Path.GetFileNameWithoutExtension(markdownPath) + ".rtmd");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (File.Exists(packagePath)) throw new IOException("Package output already exists.");
            ...
            await using (var stream = new FileStream(temporaryPath, FileMode.CreateNew, FileAccess.ReadWrite, FileShare.None, 64 * 1024, FileOptions.Asynchronous))
            {
                using (var archive = new ZipArchive(stream, ZipArchiveMode.Create, leaveOpen: true))
                {
                    ...
                    foreach (var file in Directory.EnumerateFiles(workspacePath, "*", SearchOption.AllDirectories)
                                 .OrderBy(path => Path.GetRelativePath(workspacePath, path), StringComparer.Ordinal))
                    {
                        var relative = Path.GetRelativePath(workspacePath, file).Replace(Path.DirectorySeparatorChar, '/');
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(packagePath)) throw new FileNotFoundException("RTMD package was not found.", packagePath);
        ...
        try
        {
            ...
            var workspaceDirectories = Directory.EnumerateDirectories(staging, "*.rtmd", SearchOption.TopDirectoryOnly).ToArray();
            if (markdownFiles.Length != 1 || workspaceDirectories.Length != 1)
                throw new InvalidDataException("Package must contain exactly one Markdown file and one RTMD workspace.");
    ...
    private static string ValidateEntry(ZipArchiveEntry entry, ISet<string> seen)
    {
