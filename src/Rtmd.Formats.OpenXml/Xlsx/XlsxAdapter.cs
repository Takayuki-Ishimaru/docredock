...
namespace Rtmd.Formats.OpenXml.Xlsx;
...
public sealed class XlsxAdapter
{
    ...
    public XlsxExtractionResult Extract(Stream source)
    {
        ...
        foreach (var sheet in workbook)
        {
            ...
            var worksheet = new XlsxWorksheetRecord(sheet.Name, sheet.PartUri, cells, used.Range, used.MinRow, used.MaxRow, used.MinColumn, used.MaxColumn, mergedRanges, drawingShapes);
    ...
    public XlsxRestoreResult Restore(Stream original, XlsxPatchPlan plan)
    {
        ...
        var warnings = new List<string>
        {
            "Cell edits require formula recalculation; RoundHound requested a full calculation on the next workbook open without evaluating formulas."
        };
        if (plan.Edits.Any(edit => edit.Formula is not null) && package.ContainsKey("xl/calcChain.xml"))
        {
            ...
            warnings.Add("Formula edits invalidated the calculation chain; no formula was evaluated by RoundHound.");
        }
