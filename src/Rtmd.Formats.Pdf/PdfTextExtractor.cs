...
public static class PdfTextExtractor
{
    ...
    private static IEnumerable<string> ReadStreams(byte[] bytes, string latin, PdfExtractionOptions options, IReadOnlySet<int> contentObjectIds)
    {
        ...
        while ((offset = IndexOf(bytes, marker, offset)) >= 0)
        {
            ...
            {
                payload = Inflate(payload, options.MaxExpandedStreamBytes);
            }
        ...
        IReadOnlyDictionary<string, PdfToUnicodeMap> fontMaps)
    {
        ...
        try
        {
            ...
            {
                var preceding = content[cursor..match.Index];
                var font = Regex.Match(preceding, @"/(?<font>[A-Za-z][A-Za-z0-9_.-]*)\s+(?:[-+]?\d+(?:\.\d*)?|\.\d+)\s+Tf\b", RegexOptions.None, options.EffectiveRegexTimeout);
                if (font.Success) currentFont = font.Groups["font"].Value;
                if (Regex.IsMatch(preceding, @"\bBT\b", RegexOptions.NonBacktracking, options.EffectiveRegexTimeout)) (x, y) = (0, 0);
                foreach (Match td in Regex.Matches(preceding, $@"({NumberPattern})\s+({NumberPattern})\s+Td", RegexOptions.NonBacktracking, options.EffectiveRegexTimeout))
                    if (double.TryParse(td.Groups[1].Value, out var dx) && double.TryParse(td.Groups[2].Value, out var dy)) (x, y) = (x + dx, y + dy);
                foreach (Match tm in Regex.Matches(preceding, $@"({NumberPattern})\s+({NumberPattern})\s+({NumberPattern})\s+({NumberPattern})\s+({NumberPattern})\s+({NumberPattern})\s+Tm", RegexOptions.NonBacktracking, options.EffectiveRegexTimeout))
                ...
                var actualText = FindActiveActualText(preceding, options.EffectiveRegexTimeout);
                ...
                if (text.Length > 0)
                    regions.Add(new PdfTextRegion(text, new Geometry("pdf-user-space", x, y, Math.Max(1, text.Length * 6), 12), regions.Count));
                x += text.Length * 6;
    ...
    private static string DecodeString(string value, PdfToUnicodeMap? map = null)
    {
        ...
        {
            if (inner[i] != '\\' || i + 1 >= inner.Length) { raw.Add((byte)inner[i]); continue; }
            var escaped = inner[++i];
            raw.Add((byte)(escaped switch { 'n' => '\n', 'r' => '\r', 't' => '\t', 'b' => '\b', 'f' => '\f', '(' => '(', ')' => ')', '\\' => '\\', _ => escaped }));
        }
    ...
    private static IEnumerable<PdfObjectStream> ReadObjectStreams(byte[] bytes, string latin, PdfExtractionOptions options)
    {
        ...
        while ((offset = IndexOf(bytes, marker, offset)) >= 0)
        {
            ...
            if (header.Contains("/FlateDecode", StringComparison.Ordinal)) payload = Inflate(payload, options.MaxExpandedStreamBytes);
