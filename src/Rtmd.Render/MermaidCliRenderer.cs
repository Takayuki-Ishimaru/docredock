...
public sealed class MermaidCliRenderer : IMermaidRenderer
{
    ...
    public async Task<byte[]> RenderPngAsync(string source, MermaidRenderRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (source.Contains("%%{", StringComparison.Ordinal)) throw new InvalidDataException("Mermaid init directives are not accepted; RoundHound supplies a strict renderer configuration.");
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), "rtmd-mermaid", Guid.NewGuid().ToString("N"));
