...
public sealed class MarkdownRenderer
{
    ...
    {
        var clean = RtmdProjectionCleaner.Clean(markdown);
        return RenderDocumentAsync(MarkdownAstParser.Parse(clean), format, outputPath, options, cancellationToken,
            RtmdProjectionCleaner.IsRtmdProjection(markdown), markdown.Any(c => c > 127));
    }
    ...
    public async Task<RenderResult> RenderDocumentAsync(MarkdownDocument document, RenderFormat format, string outputPath, RenderOptions? options = null, CancellationToken cancellationToken = default,
        bool sanitizedRtmd = false, bool containsNonAscii = false)
    {
        ...
        if (sanitizedRtmd) warnings.Add("RTMD control metadata was removed before rendering.");
    ...
    private static void WriteDocx(MarkdownDocument document, string path, RenderOptions? options)
    {
        ...
        var imageRelationships = string.Concat(diagrams.Select((_, index) => $"<Relationship Id=\"rIdRtmdMermaid{index + 1}\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/image\" Target=\"media/rtmd-mermaid-{index + 1}.png\"/>"));
        ...
        for (var index = 0; index < diagrams.Length; index++) Add(archive, $"word/media/rtmd-mermaid-{index + 1}.png", diagrams[index].Image.PngBytes);
    }
                    ...
                    new XElement(a + "graphic",
                        new XElement(a + "graphicData", new XAttribute("uri", "http://schemas.openxmlformats.org/drawingml/2006/picture"),
                            new XElement(pic + "pic",
                                new XElement(pic + "nvPicPr", new XElement(pic + "cNvPr", new XAttribute("id", index), new XAttribute("name", $"rtmd-mermaid-{index}.png"), new XAttribute("descr", diagram.AltText)), new XElement(pic + "cNvPicPr")),
                                new XElement(pic + "blipFill", new XElement(a + "blip", new XAttribute(r + "embed", $"rIdRtmdMermaid{index}")), new XElement(a + "stretch", new XElement(a + "fillRect"))),
    ...
    private static XElement WordParagraph(XNamespace w, string inlineMarkdown, string? styleId = null, bool numbered = false)
    {
        ...
        foreach (var run in RtmdInlineMarkdown.Parse(inlineMarkdown).Runs) paragraph.Add(WordRun(w, run));
    ...
    private static void WritePptx(MarkdownDocument document, string path, RenderOptions? options)
    {
        ...
        var title = PlainInlineText(firstHeading?.Text ?? options?.Title ?? "RoundHound Document");
        ...
        for (var index = 0; index < diagrams.Length; index++) Add(archive, $"ppt/media/rtmd-mermaid-{index + 1}.png", diagrams[index].Image.PngBytes);
    }
    ...
    private static XElement SlidePicture(XNamespace p, XNamespace a, XNamespace r, MarkdownDiagram diagram, int index, long slotTop, long slotHeight)
    {
        ...
        return new XElement(p + "pic",
            ...
            new XElement(p + "blipFill", new XElement(a + "blip", new XAttribute(r + "embed", $"rIdRtmdMermaid{index}")), new XElement(a + "stretch", new XElement(a + "fillRect"))),
    ...
    private static XDocument PptxMaster(XNamespace p, XNamespace a, XNamespace r)
    {
        ...
        return new XDocument(new XDeclaration("1.0", "UTF-8", "yes"), new XElement(p + "sldMaster",
            ...
            new XElement(p + "cSld", new XAttribute("name", "RoundHound Master"), new XElement(p + "spTree",
                new XElement(p + "nvGrpSpPr", new XElement(p + "cNvPr", new XAttribute("id", "1"), new XAttribute("name", string.Empty)), new XElement(p + "cNvGrpSpPr"), new XElement(p + "nvPr")),
                new XElement(p + "grpSpPr", new XElement(a + "xfrm")))),
    ...
    private static XDocument PptxTheme(XNamespace a)
    {
        ...
        var colors = new XElement(a + "clrScheme", new XAttribute("name", "RoundHound"),
        ...
        var fonts = new XElement(a + "fontScheme", new XAttribute("name", "RoundHound"), FontSet("majorFont"), FontSet("minorFont"));
        var format = new XElement(a + "fmtScheme", new XAttribute("name", "RoundHound"),
        ...
        return new XDocument(new XDeclaration("1.0", "UTF-8", "yes"), new XElement(a + "theme", new XAttribute(XNamespace.Xmlns + "a", a), new XAttribute("name", "RoundHound"), new XElement(a + "themeElements", colors, fonts, format)));
    }
    ...
    private static void WriteXlsx(MarkdownDocument document, string path, RenderOptions? options)
    {
        ...
        var worksheet = new XElement(x + "worksheet", new XAttribute(XNamespace.Xmlns + "r", r),
            ...
            diagrams.Length == 0 ? null : new XElement(x + "drawing", new XAttribute(r + "id", "rIdRtmdDrawing1")));
        Add(archive, "xl/worksheets/sheet1.xml", new XDocument(worksheet).ToString(SaveOptions.DisableFormatting));
        if (diagrams.Length == 0) return;

        Add(archive, "xl/worksheets/_rels/sheet1.xml.rels", "<?xml version=\"1.0\"?><Relationships xmlns=\"http://schemas.openxmlformats.org/package/2006/relationships\"><Relationship Id=\"rIdRtmdDrawing1\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/drawing\" Target=\"../drawings/drawing1.xml\"/></Relationships>");
        var imageRelationships = string.Concat(diagrams.Select((_, index) => $"<Relationship Id=\"rIdRtmdMermaid{index + 1}\" Type=\"http://schemas.openxmlformats.org/officeDocument/2006/relationships/image\" Target=\"../media/rtmd-mermaid-{index + 1}.png\"/>"));
        ...
        for (var index = 0; index < diagrams.Length; index++) Add(archive, $"xl/media/rtmd-mermaid-{index + 1}.png", diagrams[index].Image.PngBytes);
    }
    ...
    private static XDocument XlsxDrawing(IReadOnlyList<XlsxDiagramPlacement> placements, XNamespace r)
    {
        ...
        var anchors = placements.Select(placement => new XElement(xdr + "oneCellAnchor",
                ...
                new XElement(xdr + "pic",
                    ...
                    new XElement(xdr + "blipFill", new XElement(a + "blip", new XAttribute(r + "embed", $"rIdRtmdMermaid{placement.Index}")), new XElement(a + "stretch", new XElement(a + "fillRect"))),
    ...
    private static string PlainInlineText(string value) => string.Concat(RtmdInlineMarkdown.Parse(value).Runs.Select(run => run.Text));
        ...
        private sealed class TrueTypeFont
        {
            ...
            public sealed class EncodingTable
            {
                ...
                public byte[] BuildToUnicodeCMap()
                {
                    ...
                    builder.Append("/CMapName /RTMD-UTF16 def\n/CMapType 2 def\n1 begincodespacerange\n<0000> <FFFF>\nendcodespacerange\n");
