...
namespace Rtmd.Ocr.Tesseract;

/// <summary>
/// Uses Apple's on-device Vision framework through the checked-in Swift
/// helper. This is intentionally a separate provider: Tesseract remains the
/// portable primary provider and its unavailable status is never hidden.
/// </summary>
public sealed class VisionOcrEngine : IOcrEngine
{
    ...
    public async ValueTask<OcrAttemptResult> RecognizeAsync(OcrInput input, OcrOptions options, CancellationToken cancellationToken)
    {
        ...
        var tempRoot = Path.Combine(Path.GetTempPath(), "rtmd-ocr", Guid.NewGuid().ToString("N"));
...
/// <summary>Tries local Tesseract first, then macOS Vision only when unavailable.</summary>
...
{
    public static IOcrEngine CreateDefault() => new FallbackOcrEngine(new TesseractOcrEngine(), new VisionOcrEngine());
}
