...
10. プレーンテキストのblock（`rich-text=inline-v1`のないparagraph、heading、list-item、quote、shape、表セルなど）は、Rich Textと同じ規則でエスケープされている。`\*`、`\_`、`\~`、`` \` ``、`\[`、`\]`、`&amp;`、`&lt;`、`&gt;`、行頭の`\#`、`\-`、`1\.`のような表記は元の`* _ ~ `` ` `` [ ] & < > # - 1.`を表す。既存のエスケープ記号を削除・展開せず、新しく`* _ ~ `` ` `` [ ] & < >`や行頭の`#`、`- `、`+ `、`1.`/`1)`、`---`/`===`を書く場合も同じ規則でエスケープする。`\[LABEL\](url)`のような表記は本文中のただの文字列であり、リンクにしたい場合を除いてエスケープを外さない。
