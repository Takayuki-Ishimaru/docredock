...
`rich-text=inline-v1`のないプレーンテキストのblock（`paragraph`、`heading`、`list-item`、`quote`、`shape`、表セルなど）も、Rich Textの本文と同じ文字集合でエスケープされます。

- `\`、`*`、`_`、`~`、`` ` ``、`[`、`]`はバックスラッシュ（例: `\*`、`\[`）、`&`、`<`、`>`は実体参照（`&amp;`、`&lt;`、`&gt;`）でエスケープされます。
- `[`と`]`をエスケープすることで、原文に文字列として書かれた`[text](url)`、`![alt](path)`、`[ref][id]`、`[id]: url`がリンク・画像・参照定義として解釈されるのを防ぎます（特に`[id]: url`の行は、エスケープしないと参照定義として吸収され表示から消えます）。`(`、`)`、`!`、`:`は読みやすさのためエスケープしません。`https://example.com`のような裸のURLもそのまま出力します。実際のハイパーリンクや画像は、エスケープ済みのラベルを生の`[` `]`で囲んで出力するため従来どおり機能します。
- 複数行の本文では、行頭の`#`、`- `/`+ `、`1.`/`1)`、``` ``` ```や`~~~`のフェンス、`---`/`===`だけの行も、見出し・リスト・コードフェンス・区切り線として再解釈されないようバックスラッシュでエスケープされます（例: `1\.`）。`` ` ``や`~`、`>`は上記の基本エスケープで既に無害化されるため、追加のエスケープは不要です。
- 表セル（`table`、`sheet-table`）も同じ文字集合でエスケープされ、既存の`|`と改行（`<br>`）の扱いはそのままです。
- `sheet-table`のセルや`cell`行（`- **A1:**`）で数式・表示値・`=`始まりの文字列がcode spanとして包まれる場合、包み用のバッククォートは常に生のまま出力され、セル値自体に含まれる`` ` ``は`` \` ``としてエスケープされます。値がそのまま`` `x` ``のような見た目でも、生のバッククォートだけを包みとみなすことで区別して復元できます。

本文を編集する際は、この規則に従ってエスケープしてください。既存の`\*`のようなエスケープ記号を残したまま前後に文字を追加するだけであれば、エスケープ記号はそのまま保持して構いません。

`code-block`と`diagram`（mermaid）のfenceは固定長ではなく、本文中に含まれる最長のバッククォート連続より長く（最低3）自動調整されます。fence内の本文自体はエスケープされず、そのまま保持されます。
