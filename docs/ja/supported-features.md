...
DocRedock v0.2.8 Public Betaでは、デスクトップGUIでDOCX、XLSX、PPTX、PDFをローカルの**閲覧用Markdown**へ変換する操作をサポートします。
...
v0.2.8では、PDF工程表の長い塗りつぶしバーと、線から分離した矢印の先端による余分な行・列の発生を抑えます。未解決の表・図には、OCRとは独立して原本照合用のページ画像を添付できます（PDF rasterizerが必要）。GUIは警告付き完了を主表示で知らせ、OCR結果にはエンジンが返す信頼度と位置を表示します。操作方法は[利用ガイド](user-guide.md)を参照してください。
...
この文書が利用者向けサポート範囲の正本です。[v0.2.8リリースノート](../../release-docs/RELEASE_NOTES_v0.2.8.md)、[利用ガイド](user-guide.md)、[実験機能](experimental-features.md)、[セキュリティとプライバシー](security-and-privacy.md)も参照してください。
